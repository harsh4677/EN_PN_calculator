...
const calculateNutrients = (tdee: number | null, weight: string, selectedCondition: string) => {
  ...
  fatVolume = fats * 1.1;
    ...
    // Calculate Dextrose, Amino Acids, and Lipids in ml
    const dextrose = (carbohydrate * 100) / 20;
    const aminoAcids = (protein * 100) / 20;
    const lipids = (fats * 100) / 20;
  
    // Calculate Total Macronutrients in ml
    const totalMacronutrients = dextrose + aminoAcids + lipids;
  
    return { carbohydrate, protein, fats, fatVolume, fluidReq, dextrose, aminoAcids, lipids, totalMacronutrients };
  };

import React, { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import PNNextStepButton from "@/components/PNNextStepButton";
  ...
  return (
    <div className="flex flex-col min-h-screen w-screen bg-gradient-to-b from-gray-800 to-gray-900">
      ...
      <main className="flex-grow overflow-y-auto flex items-center justify-center">
        <div className="flex flex-col gap-6 w-fit bg-gray-700 p-10 rounded-lg shadow-2xl">
          ...
          <div className="flex flex-col items-center gap-4 mt-4">
            ...
            {nutrientValues && (
              <div className="bg-gray-800 p-6 rounded-lg shadow-lg mt-6">
                <p className="text-white text-lg">Carbohydrates: {nutrientValues.carbohydrate.toFixed(2)} g</p>
                <p className="text-white text-lg">Proteins: {nutrientValues.protein.toFixed(2)} g</p>
                <p className="text-white text-lg">Fats: {nutrientValues.fats.toFixed(2)} g</p>
                <p className="text-white text-lg">Fat Volume: {nutrientValues.fatVolume.toFixed(2)} ml</p>
                <p className="text-white text-lg">Fluid Requirement: {nutrientValues.fluidReq.toFixed(2)} ml</p>
              </div>
            )}

            {nutrientValues && (
              <div className="bg-gray-800 p-6 rounded-lg shadow-lg mt-6">
                <p className="text-white text-lg">Dextrose: {nutrientValues.dextrose.toFixed(2)} ml</p>
                <p className="text-white text-lg">Amino Acids: {nutrientValues.aminoAcids.toFixed(2)} ml</p>
                <p className="text-white text-lg">Lipids: {nutrientValues.lipids.toFixed(2)} ml</p>
                <p className="text-white text-lg">Total Macronutrients: {nutrientValues.totalMacronutrients.toFixed(2)} ml</p>
              </div>
            )}
